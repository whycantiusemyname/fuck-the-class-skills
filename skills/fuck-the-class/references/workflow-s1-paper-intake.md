...
- 写入：`10_题库/<卷名>题面整理.md`
- 每个 PDF，包括扫描 PDF，都需要已验证 `$pdf-to-markdown` cache：`90_缓存/pdf-to-markdown/<source-stem>/completion.json`

## 步骤

1. 若源文件是 PDF，包括扫描 PDF，严格执行 `pdf-ingestion.md` 的 Gate 0 和 Gate 1。只有当前源文件、认证 Markdown 和零 unresolved items 通过独立 completion verifier 后才能继续。不要消费 draft、单个 chunk、`source/`、`repaired/` 或自然语言成功声明。独立照片可直接读图，并保留视觉不确定性等待确认。
...
7. 写文档级 `paper_type`、`academic_year`；写题目级 `chapter`、`question_type`、`question_form`、`ocr_status`。PDF completion 只证明转换 Markdown 可消费，不自动证明逐题 source verification。具体来源、公式、图或 OCR 不确定时用 `待复核`；只经过转换清理但未逐题核对时用 `已做结构修复`；只有题面已经对照原 PDF 或可信来源后才能用 `已对照 PDF 复核`。
...
PDF completion gate 通过前，不拆题、不改 `_标签库.md`、不创建或更新 `10_题库/`。转换 subagent 可用且正在运行时，S1 parent 不写其转换产物，不替其 finalize；没有 subagent 能力时，main agent 也必须按同一 gate 执行。S1 parent 永远不手工创建或修复 `completion.json`。唯一可消费的 PDF 文本是 certificate 当前 `final_markdown`；禁止消费 `draft.md`、`.mineru.md`、`source/`、`repaired/`。S1 不写答案或解析；需要解答时转 S9。
