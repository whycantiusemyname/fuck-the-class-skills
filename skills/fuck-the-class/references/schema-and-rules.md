...
├── 20_知识/          # 主干讲解、AI tutor grounding、S10启动卡和问答种子
├── 30_我的数据/
│   ├── inbox/        # 新手写截图
│   ├── archive/      # 已处理截图
...
| S8 courseware digest | 章节范围、课件映射、主线/补充判断 | 源课件、可选 `课程口径.md`、现有 `20_知识/`、可选标签库和考频 | `20_知识/第XX章_主题_主干讲解.md`、`第XX章_主题_grounding.md`、`第XX章_主题_S10启动卡.md`、同步 README/框架、必要 cache |
...
S8 旧文件名兼容：`第XX章_入门讲解.md` 视为旧版启动卡，`第XX章_主干重点.md` 视为旧版主干讲解。新生成默认使用 `主干讲解`、`grounding`、`S10启动卡` 三件套；默认 S8 不得只生成 grounding 或 S10启动卡。
...
S10 追加事件时优先使用 `scripts/append_learning_event.py`，不要手写 JSONL。推荐流程：先把事件对象写到 `<course-root>/90_缓存/s10-event.json`，再运行 `python <skill>/scripts/append_learning_event.py --course-root <course-root> --event-file <course-root>/90_缓存/s10-event.json`，最后运行 S10 validator。该脚本会自动补 `event_id`、`time`、`event_signature`，并用签名去重。
