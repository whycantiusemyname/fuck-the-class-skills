...
- 课程边界来自 `课程口径.md`、题库、S8 `grounding.md` 和用户当前指令；学生主阅读材料优先使用 S8 `主干讲解.md`，快速进入问答时才使用 `S10启动卡.md`。不要引入课外可考内容。
...
- 学生刚读完 S8：若学生未建立主线，先沿 `主干讲解.md` 的认知坡道讲一轮；若学生已读过主干讲解，可用 `S10启动卡.md` 快速进入 probe 或做题。
