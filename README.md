...
PDF 入库带独立完成门禁：`s1_intake_gate.py` 会绑定源 PDF、`completion.json`、认证后的最终 Markdown、题库输出路径及各自 SHA-256。草稿、中间修复分段、重复源文件和提交后被修改的产物都会被拒绝。`probe_pdf_progress.py` 还可以根据转换检查点客观判断子 Agent 是否真正推进。
...
当前 S2 使用 schema 2.0，支持相邻 H1-H6 卷级元数据、统一的 Decimal half-up 百分比取整和 `importance_role`。`analyze_frequency_trends.py` 生成分析 JSON 后，`render_frequency_views.py` 确定性生成考频矩阵与主题题表，并可根据输入指纹和逐字节内容检查人工漂移。
...
`s3_batch_gate.py` 使用批次日志和隐藏 marker 防止重复追加；即使流程在写入记录后、移动图片前中断，也能从已有状态恢复，而不会重复记账。

适合：完成一套卷或一轮练习后，把结果持续积累进个人数据。

### S4：生成当前练习队列

根据题库、重要度、近期趋势、做题历史和未解决问题，生成此刻应该完成的练习队列。它只负责当前队列，不擅自扩展成日程表。标记为 `ocr_status: 待复核` 的题目会被硬跳过；在“只出红区”模式下没有合格题目时，会明确生成空队列，不会偷偷降级成普通练习。
...
根据题库、趋势分析、做题记录和卡点，生成最后阶段可直接使用的冲刺材料，包括考前检查单、启动练习和提醒卡。每条“起手”只能来自非存疑的 S9 解答或用户确认记录；没有可信来源时保留题目并标记 `待 S9`，不会现场编造解题提示。

适合：考试临近，需要把复习范围压缩成最后一套行动材料。

### S7：学习对话提取

从导出的 AI 学习对话中提取真正暴露理解障碍的片段，保留关键原话和最终解释，把尚未解决的问题追加到卡点清单，而不是把整段聊天简单总结一遍。逐字引用会绑定源文件、1-based 行号、源文件 SHA-256 和引用 SHA-256，自检发现任何改写或错位都会阻断写入。

适合：平时通过 AI 学概念，希望把对话里的卡点沉淀下来。

### S8：课件与章节梳理

把 PDF、PPT/PPTX 等章节课件整理成可独立阅读的主干笔记，提炼概念关系、公式条件、典型问题和章节重点，写入课程知识区。`s8_digest_gate.py` 会把源课件、PDF 证书和章节输出绑定到 digest manifest；源文件或笔记变化后，旧 manifest 会失效，避免把过期梳理误报为完成。

适合：老师的课件信息密度高、顺序跳跃，复习时需要一份连贯版本。

### S9：核验解答

为题库中的指定题目补充经过检查的解答，并以折叠块写在题目下方。解答工作不会擅自改写题干、标签或题目来源，也会拒绝为 `ocr_status: 待复核` 的题目配解答，先将题面不确定性路由回 S1。
...
- `ocr_status` 固定为 `待复核`、`已做结构修复`、`已对照 PDF 复核` 三态；旧题目缺失该字段时只按 `已做结构修复` 读取，不会擅自回写升级。
- 卷级元数据必须紧邻第一处 H1-H6 标题，避免误读后续题目元数据。
- 百分比和百分点统一使用 Decimal `ROUND_HALF_UP`，正负半值采用一致规则。
- `validate_course_artifacts.py` 统一检查控制字符、LaTeX 损坏、断链、标题锚点、vault 根相对链接、逐字引用证据和派生文件漂移。
- 裸数学表达只产生警告；真正的控制字符、公式损坏、错误链接或证据失配才会阻断完成。
- S1、S3 和 S8 必须分别提供有效的 intake、batch 和 digest manifest，文件存在本身不再等于工作流完成。
...
`fuck-the-class` 当前包含 33 项测试，覆盖数据协议、考频与趋势分类、输入指纹、确定性渲染、派生文件漂移、统一链接检查、S1 入库门禁、PDF 进度探测、S3 中断恢复、S4/S9 OCR 状态约束、S6 起手来源和 S8 digest manifest。仓库中的 `pdf-to-markdown` 另有独立的转换与完成证书测试。
